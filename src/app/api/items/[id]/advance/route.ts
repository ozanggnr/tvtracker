...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (item.itemType === "TV_SERIES" && item.externalId?.startsWith("tmdb_tv_")) {
      ...
      if (!tmdbRes.ok) {
        ...
        const updated = await prisma.trackedItem.update({
          where: { id },
          data: { currentEpisode: newEp, progress: (item.progress || 0) + 1 }
        });
      ...
      let curSeason = item.currentSeason || 1;
      let curEp = item.currentEpisode || 0;
    ...
    // Fallback for BOOK or other types
    const updated = await prisma.trackedItem.update({
      where: { id },
